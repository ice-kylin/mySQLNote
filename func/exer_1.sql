...
# 2. 查询员工号、姓名、工资、以及工资提高 20% 后的结果（new salary）
SELECT
    employee_id,
    last_name,
    salary,
...
SELECT
    CONCAT_WS(', ', employee_id, last_name, salary) "out_put"
    FROM
        employees;
...
SELECT
    ...
    last_name,
    hire_date,
    YEAR(FROM_DAYS(DATEDIFF(CURDATE(), hire_date))) AS years_worked,
...
# 6. 查询员工姓名、hire_date、department_id，满足以下条件：雇用时间在 1997 年之后、department_id
# 为 80 或 90 或 110、commission_pct 不为空
SELECT
    last_name,
    hire_date,
    department_id,
